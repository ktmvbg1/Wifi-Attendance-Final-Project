...
def login(session, username, password):
    ...
    if user:
        if not verify_password(password, user.password):
            ...
            return (True, create_access_token(data, datetime.timedelta(hours= 48)))
    elif user == None:
        return (False, "User Not Found")

def check_access_course(session, user_id, course_id):
	permission = session.query(CourseUsers).filter_by(user_id = user_id, course_id = course_id).first()
	if not permission:
		return False
	return True
